...
let userScore = 0;
let compScore = 0;
let roundsPlayed = 0; // Tracks the current round number
let matchLimit = 0; // Stores the maximum number of rounds
const choices = ['fire', 'ice', 'air'];
...
/** Enables or disables the choice buttons and limit input/button */
...
function determineWinner(userChoice, compChoice) {
    ...
    // Win conditions: Fire beats Ice, Ice beats Air, Air beats Fire
    if (
        (userChoice === 'fire' && compChoice === 'ice') ||
        (userChoice === 'ice' && compChoice === 'air') ||
        (userChoice === 'air' && compChoice === 'fire')
    ) {
        return 'win';
    } else {
        return 'lose';
...
function displayResult(userChoice, compChoice, outcome) {
    ...
    resultEl.textContent = `${emoji} Round ${roundsPlayed}/${matchLimit}: You picked ${userChoice.toUpperCase()}, Computer picked ${compChoice.toUpperCase()}. ${message}`;
    
    // Add result to history
    const historyItem = document.createElement('div');
    historyItem.textContent = `Round ${roundsPlayed}: ${message} (${userChoice} vs ${compChoice})`;
...
function resetGame() {
    ...
    historyEl.innerHTML = '';
    toggleChoices(false); // Disable choices
}
...
setLimitBtn.addEventListener('click', () => {
    ...
    if (limit > 0 && limit <= 50) { // Enforce a reasonable maximum
        matchLimit = limit;
